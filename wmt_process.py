...
def load_wmt_nonlabel(tokenizer,
    ...
    task_prompt_map = {
        ...
        "zh-en": "Translate the sentence from Chinese to English Please.",
    }
...
def load_wmt_datals(tokenizer,
    ...
    if use_local_model_with_wrmk is None:
        ...
        if use_opensource==0:
            ...
            V,
            lm_tokenizer,
            victim="/root/meta-llama/Meta-Llama-3-70B-Instruct",
            )
    else:
...
def infer_wmt(modelname, task_name, res_pth,
    ...
    pretrained_ls=[
        "/mnt/petrelfs/share_data/ai4good_shared/models/meta-llama/llama3-8b-instruct",
    ...
    if modelname=="gpt-3.5-turbo-1106":
        ...
        for d in tqdm(sets,total=test_set_take_num):
            ...
            # 在生成前添加
            attention_mask = (inps_idx != tokenizer.pad_token_id).long().to("cuda")
            res = model.generate(
                inps_idx,
...
def evaluation_datas():
    ...
    base_model_name="/mnt/petrelfs/share_data/ai4good_shared/models/meta-llama/llama3-8b-instruct"
    res_dict = {}
    dir_p = "./wmt16_res/"
                ...
                for train_num in train_nums:
                    for m in mls:
                        ...
                        for itime in train_times:
                            ...
                            prefix = f"./wmt16_ckpts/WMT_Hyperpara"
                            ckpt = (
                                prefix
